...
def PROTEIN(FILE):
    ...
    for i in range(1, pose.total_residue() + 1):
        ...
        for j in range(1, residue.natoms() + 1):
            ...
            atom_name = re.sub(r'[0-9]', '', atom_name)[0].upper()
            if atom_name in element_map:
                atom_name = element_map[atom_name]
            else:
                atom_name = -1
            xyz = atom.xyz()
            atom_names.append(atom_name)
            Position.append([xyz.x, xyz.y, xyz.z])
    
    edge_index = []
    hbonds = pose.get_hbonds()
    for i in range(1, hbonds.nhbonds() + 1):
        hbond = hbonds.hbond(i)
        edge_index.append([hbond.acc_res() - 1, hbond.don_res() - 1])
        edge_index.append([hbond.don_res() - 1, hbond.acc_res() - 1])
    
    NUMR = 0
    total_residues = pose.total_residue()
    for res in range(1, total_residues + 1):
        edge_list = []
        residue1 = pose.residue(res)
        NA = residue1.natoms()
        
        for atom1 in range(1, NA + 1):
            for atom2 in range(atom1, NA + 1):
                atom1_id = AtomID(atom1, res)
                atom2_id = AtomID(atom2, res)
                if conformation.atoms_are_bonded(atom1_id, atom2_id):
                    edge_list.append([(atom1 + NUMR) - 1, (atom2 + NUMR) - 1])
                    edge_list.append([(atom2 + NUMR) - 1, (atom1 + NUMR) - 1])
        
        edge_index.extend(edge_list)
        if res < total_residues:
            edge_index.append([NA-1 + NUMR, NA + NUMR])
            edge_index.append([NA + NUMR, NA-1 + NUMR])
        NUMR += NA
    ...
    edge_attr = torch.full((edge_index.shape[1], 3), 0.5)
